...
u32 cudbg_get_entity_length(struct adapter *adap, u32 entity)
{
	...
	case CUDBG_MPS_TCAM:
		len = sizeof(struct cudbg_mps_tcam) *
		      adap->params.sfos.mps_tcam_size;
		break;
	case CUDBG_VPD_DATA:
...
#undef ulp_region
	...
	for (i = 0; i < padap->params.sfos.nchan; i++) {
		if (CHELSIO_CHIP_VERSION(padap->params.chip) > CHELSIO_T5)
			lo = t4_read_reg(padap,
				...
				struct cudbg_error *cudbg_err)
{
	...
	vf_count = padap->params.sfos.vfcount;
			       ...
			       struct cudbg_error *cudbg_err)
{
	...
	if (CHELSIO_CHIP_VERSION(padap->params.chip) > CHELSIO_T5 &&
		...
		for (i = 0; i < padap->params.sfos.vfcount; i++)
			cudbg_read_sge_qbase_indirect_reg(padap, sge_qbase,
							  i, false);

		sge_qbase->vfcount = padap->params.sfos.vfcount;
	}
				    ...
				    struct cudbg_mps_tcam *tcam, u32 idx)
{
	...
	if (tcam->repli) {
		...
		if (padap->params.sfos.mps_rplc_size > CUDBG_MAX_RPLC_SIZE) {
	...
	tcam->idx = idx;
	tcam->rplc_size = padap->params.sfos.mps_rplc_size;
	return rc;
}
			   ...
			   struct cudbg_error *cudbg_err)
{
	...
	n = padap->params.sfos.mps_tcam_size;
