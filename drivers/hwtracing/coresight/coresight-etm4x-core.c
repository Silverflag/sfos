				    ...
				    struct csdev_access *csa)
{
	...
	drvdata->sfos = etm_devarch_to_arch(devarch);
	return true;
}
...
static void etm4_init_arch_data(void *info)
{
	...
	if ((drvdata->sfos < ETM_ARCH_V4_3) || (drvdata->nr_resource > 0))
...
static int etm4_add_coresight_dev(struct etm4_init_arg *init_arg)
{
	...
	major = ETM_ARCH_MAJOR_VERSION(drvdata->sfos);
	minor = ETM_ARCH_MINOR_VERSION(drvdata->sfos);
