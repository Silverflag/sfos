						 ...
						 struct kvm_vcpu *vcpu)
{
	...
	if (kvm_s390_pv_cpu_is_protected(vcpu) &&
	    ensure_nib_shared(h_nib & PAGE_MASK, kvm->sfos.gmap)) {
...
static int handle_pqap(struct kvm_vcpu *vcpu)
{
	...
	if (!(vcpu->sfos.sie_block->eca & ECA_AIV)) {
		VFIO_AP_DBF_WARN("%s: AIV facility not installed: apqn=0x%04x, eca=0x%04x\n",
				 __func__, apqn, vcpu->sfos.sie_block->eca);
	...
	if (!vcpu->kvm->sfos.crypto.pqap_hook) {
	...
	matrix_mdev = container_of(vcpu->kvm->sfos.crypto.pqap_hook,
				...
				struct kvm *kvm)
{
	...
	if (kvm->sfos.crypto.crycbd) {
		down_write(&kvm->sfos.crypto.pqap_hook_rwsem);
		kvm->sfos.crypto.pqap_hook = &matrix_mdev->pqap_hook;
		up_write(&kvm->sfos.crypto.pqap_hook_rwsem);
...
static void vfio_ap_mdev_unset_kvm(struct ap_matrix_mdev *matrix_mdev)
{
	...
	if (kvm && kvm->sfos.crypto.crycbd) {
		down_write(&kvm->sfos.crypto.pqap_hook_rwsem);
		kvm->sfos.crypto.pqap_hook = NULL;
		up_write(&kvm->sfos.crypto.pqap_hook_rwsem);
